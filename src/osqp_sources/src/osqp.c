...
void osqp_set_ROC_settings(OSQPSettings *settings)
{
  ...
  settings->max_iter = MAX_ITER_ROC;              /* maximum iterations to
                                                 take */
  settings->eps_abs = (c_float)EPS_ABS_ROC;       /* absolute convergence
                                                 tolerance */
  settings->eps_rel = (c_float)EPS_REL_ROC;       /* relative convergence
                                                 tolerance */
  settings->eps_prim_inf = (c_float)EPS_PRIM_INF; /* primal infeasibility
                                                     tolerance */
  settings->eps_dual_inf = (c_float)EPS_DUAL_INF; /* dual infeasibility
  ...
  settings->polish_refine_iter = POLISH_REFINE_ITER; /* iterative refinement
                                                        steps in polish */
  settings->verbose = VERBOSE_ROC;                   /* print output */
#endif                                               /* ifndef EMBEDDED */
  ...
  settings->check_termination = CHECK_TERMINATION;   /* Interval for evaluating
                                                        termination criteria */
  settings->warm_start = WARM_START_ROC;             /* warm starting */
...
c_int osqp_setup(OSQPWorkspace **workp, const OSQPData *data, const OSQPSettings *settings)
{
  ...
  *workp = work;
  ...
  if (settings->scaling)
  {
    ...
    //   return osqp_error(OSQP_MEM_ALLOC_ERROR);
    if (!(work->D_temp) || !(work->D_temp_A))
      return osqp_error(OSQP_MEM_ALLOC_ERROR);
    if (data->m && !(work->E_temp))
      return osqp_error(OSQP_MEM_ALLOC_ERROR);
  ...
  if (load_linsys_solver(work->settings->linsys_solver))
    return osqp_error(OSQP_LINSYS_SOLVER_LOAD_ERROR);
...
c_int osqp_cleanup(OSQPWorkspace *work)
{
  ...
  if (work)
  { // If workspace has been allocated
    // Free Data
    if (work->data)
    {
      if (work->data->P)
    ...
    if (work->rho_vec)
      c_free(work->rho_vec);
    if (work->rho_inv_vec)
      c_free(work->rho_inv_vec);
#if EMBEDDED != 1
    if (work->constr_type)
      c_free(work->constr_type);
#endif
    if (work->x)
      c_free(work->x);
    if (work->z)
                    ...
                    c_int P_new_n)
{
  ...
  if (Px_new_idx)
  { // Passing the index of elements changed
